...
Este pquete contiene las funciones basicas que le permiten a R funcionar como lengiaje: aitmetica, entrraddas y salidas, soporte de programacion, etc. su contenido esta disponible en todos los entornos de R
...
```{R vector}
#Los vecotres solo pueden tener variables del mismo tipo
...
lista <- seq(1,10)
pairList(lista)
#saca los pares, array es vector
```
...
posicion 3
R2/
varias posiciones puntuales
R3/
un rango de posiciones continuo
###Matriz
...
print("")
class(x)
#el byrow cambia si el orden es por columnas o por filas
...
x[5,3]<-30 #cambio un punto de la matriz
#si se ingresa un componente de otro tipo, como chr, me cambia todo porque las matrices solo pueden tener un tipo de variables
x
```
...
```{r attachExample2}
attach(mtcars)
#attach: todo lo que voy a usar de aqui en adelante esta en el dataset de mtcars
  summary(mpg)
  plot(mpg, disp)
  plot(mpg, wt)
detach(mtcars)
#deshace el attach, siempre es recomendable hacer esto
```
